...
function definirCartes() {
  ...
  for carte in "${CARTES_COURANTES[@]}"; do
    ...
    echo "$carte"
  done > "${JOUEUR_ID}_CARTES_COURANTES.tmp"
...
function commandeQuitter() {
    ...
    if [[ "$CARTE_A_JOUER" == "q" || "$CARTE_A_JOUER" == "QUIT" ]]; then
        echo "[INFO] | Le joueur $JOUEUR_ID a demandé à quitter la partie."
    ...
    if [[ $CARTE_A_JOUER =~ ^-?[0-9]+$ ]]; then
        for (( x=0; x<$NB_CARTES; x++ )); do
            ...
            if [ "$CARTE_A_JOUER" -eq "$CARTE_COURANTE" ]; then
                CARTE_IMPOSSIBLE_A_JOUER=false  # La carte est présente dans sa main
                break
            fi
        done

        # Si la carte n'est pas valide, affiche un message d'erreur
        if [ "$CARTE_IMPOSSIBLE_A_JOUER" = true ]; then
...
function ecouterCarteAJouer() {
    while true; do
        ...
        while $CARTE_IMPOSSIBLE_A_JOUER; do  # Tant que l'entrée utilisateur est mauvaise, on répète
...
function ecouterPipe(){
  ...
  DONNEES_ENTRANTES=$(cat $JOUEUR_ID.pipe)
...
echo "Vous êtes le joueur n°"$JOUEUR_ID
