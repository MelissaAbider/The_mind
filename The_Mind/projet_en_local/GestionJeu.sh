...
NBROBOTS=0 # Décrit le nombre de robots
TEMPS_REACTION=15
declare -i MANCHE_MAX=0 # On déclare un integer. Décrit le nombre maximum de manches
...
function InitJoueurs(){
  ...
  # On supprime les pipes existants ( normalement non nécessaire, cette fonction est juste là pendant la période de développement et sert de sécurité une fois le projet fini )
   ...
   if [ "$NBJOUEURS" -gt 0 ]; then
    # Initialisation des terminaux et des pipes pour chaque joueur
     for ((x = 0; x < NBJOUEURS; x++)); do
         xterm -e "./JoueurHumain.sh $x" & # Démarre un terminal pour chaque joueur
         mkfifo "$x.pipe" # Crée un pipe nommé pour chaque joueur
     done
  fi

  
 if [ "$NBROBOTS" -gt 0 ]; then
    # Initialisation des terminaux et des pipes pour les robots
    debut=$((MANCHE * NBJOUEURS))
    fin=$((debut + MANCHE * NBROBOTS - 1))

    for x in $(seq "$debut" "$fin"); do
        xterm -e "./JoueurRobot.sh $x" & # Démarre un terminal pour chaque robot
        mkfifo "$x.pipe" # Crée un pipe nommé pour chaque robot
    done
fi
...
function FinJeu(){
    ...
    #Appel les fonctions de génération de stats et du classement
...
function distribuerCartes() {
    ...
    for x in $(eval echo {0..$(($NBJOUEURS + $NBROBOTS - 1))}); do
...
function trierCartes() {
    ...
    for x in $(eval echo {0..$(($MANCHE * $NBJOUEURS + $MANCHE * $NBROBOTS - 1))}); do
...
function commandeQuitter() {
    if [[ "$CARTE_ENTRANTE" == "QUIT" || "$CARTE_ENTRANTE" == "q" ]]; then
...
function gererFinDeTour() {
    if [ "$INDEX_CARTE_COURANTE" -eq $((MANCHE * NBJOUEURS + MANCHE * NBROBOTS)) ]; then
        MANCHES_MAX=$((14 - 2 * (NBJOUEURS + NBROBOTS - 1)))  # Calcul du nombre maximal de manches
...
function EnvoyerMsgRobot() {  
    if [ "$NBROBOTS" -gt 0 ]; then
        ...
        for ((x = NBJOUEURS; x < NBJOUEURS + NBROBOTS; x++)); do
            echo "$ID_MSG;$MSG_A_ENVOYER" > "$x.pipe"
        done
...
function supprimerAnciensFichiers(){
  # Supprime tous les pipes / tous les fichiers tmp précédents
