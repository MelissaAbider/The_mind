...
function genererGraphique() {
    ...
    gnuplot <<- EOF
        ...
        plot "$DOSSIER_SCRIPT/donnees_reussite.txt" using 2:1 with linespoints title "Manches réussies"
EOF
}
...
function genererHistogrammeCartesPerdues() {
    ...
    }' "$FICHIER_STATS" > "$DOSSIER_SCRIPT/valeurs_echec_par_manche_groupees.txt"

    # Générer l'histogramme avec Gnuplot
    gnuplot <<- EOF
        set terminal png size 800,600
        set output "$DOSSIER_SCRIPT/histogramme_valeurs_echec_groupe.png"
        ...
        plot for [i=2:*] "$DOSSIER_SCRIPT/valeurs_echec_par_manche_groupees.txt" using i:xtic(1) title columnheader(i)
EOF

    echo "Histogramme généré : $DOSSIER_SCRIPT/histogramme_valeurs_echec_groupe.png"
}
...
function genererPDF() {
    ...
    echo "Création du fichier Markdown..."
    {
        ...
        echo "![Cartes perdues par manche]($DOSSIER_SCRIPT/histogramme_valeurs_echec_groupe.png)"
        echo ""
